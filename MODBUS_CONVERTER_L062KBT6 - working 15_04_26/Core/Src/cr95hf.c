...
int idx = 0, A5_5A_Err=0, A5_5A_Err_Count=0;
...
void build_frame(uint8_t *tx)
{
    ...
    memcpy(&tx[idx], &total_volume, 8); idx += 8;
    memcpy(&tx[idx], &forward_volume, 8); idx += 8;
    memcpy(&tx[idx], &reverse_volume, 8); idx += 8;
